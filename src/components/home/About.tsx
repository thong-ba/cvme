...
const About = () => {
  ...
  const t = homeTranslations[locale].about;
  ...
  const highlightIcons = [GraduationCap, Code, Users, Target];
  const highlightColors = ['text-indigo-600', 'text-purple-600', 'text-pink-600', 'text-emerald-600'];
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            {t.highlights.map((highlight, index) => {
              ...
              return (
                ...
                >
                  <div className={`flex h-10 w-10 items-center justify-center rounded-lg bg-gradient-to-br from-indigo-100 to-purple-100 ${colorClass} dark:from-indigo-900/30 dark:to-purple-900/30`}>
                    <Icon size={20} />
                  </div>
          ...
          <div className="space-y-4 text-base leading-7 text-slate-700 sm:text-lg sm:leading-8 dark:text-slate-300">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-6">
              <div className="flex items-start gap-3 rounded-xl border border-slate-200 bg-white/80 p-4 shadow-sm dark:border-slate-700 dark:bg-slate-800/60">
                ...
                <div>
                  <h4 className="font-semibold text-slate-900 dark:text-slate-100 mb-1">{t.valueQuality}</h4>
                  <p className="text-sm text-slate-600 dark:text-slate-400">{t.valueQualityDesc}</p>
                </div>
              </div>
              <div className="flex items-start gap-3 rounded-xl border border-slate-200 bg-white/80 p-4 shadow-sm dark:border-slate-700 dark:bg-slate-800/60">
                <Lightbulb className="text-purple-600 dark:text-purple-400 flex-shrink-0 mt-1" size={20} />
                <div>
                  <h4 className="font-semibold text-slate-900 dark:text-slate-100 mb-1">{t.valueInnovation}</h4>
                  <p className="text-sm text-slate-600 dark:text-slate-400">{t.valueInnovationDesc}</p>
                </div>
              </div>
              <div className="flex items-start gap-3 rounded-xl border border-slate-200 bg-white/80 p-4 shadow-sm dark:border-slate-700 dark:bg-slate-800/60">
                <Users className="text-pink-600 dark:text-pink-400 flex-shrink-0 mt-1" size={20} />
                <div>
                  <h4 className="font-semibold text-slate-900 dark:text-slate-100 mb-1">{t.valueCollaboration}</h4>
                  <p className="text-sm text-slate-600 dark:text-slate-400">{t.valueCollaborationDesc}</p>
                </div>
              </div>
            </div>
          </div>
